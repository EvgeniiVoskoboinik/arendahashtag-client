...
@Component({
               ...
               templateUrl: './search-select.template.html',
               styleUrls: ['./search-select.style.scss'],
               providers: [
                   {
...
export class Behavior {
    ...
    public actor: SearchSelectComponent;

    public constructor(actor: SearchSelectComponent) {
        this.actor = actor;
    }
            ...
            .filter((option: SelectItem) => {
                return stripTags(option.text).match(query) &&
                    (this.actor.multiple === false ||
                        (this.actor.multiple === true && this.actor.active.map((item: SelectItem) => item.id).indexOf(
                            option.id) < 0));
            });
...
export class ChildrenBehavior extends Behavior implements OptionsBehavior {
    ...
    public filter(query: RegExp): void {
        ...
        for (let si of this.actor.itemObjects) {
            let children: SelectItem[] = si.children.filter((option: SelectItem) => query.test(option.text));
